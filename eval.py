...
from datasets import VOCSegmentation, ADESegmentation, DGSegmentation, iSSegmentation
...
def get_argparser():
    ...
    parser.add_argument("--dataset", type=str, default='deepglobe', choices=['voc', 'ade','deepglobe', 'iSAID'], help='Name of dataset')
    ...
    parser.add_argument("--meta", action='store_true', default=False, help="enable meta block")
...
def get_dataset(opts):
    ...
    dataset_dict['test'] = dataset(opts=opts, image_set='test_val' if opts.test_on_val else 'test',
                                   transform=val_transform, cil_step=opts.curr_step)
    ...
    with torch.no_grad():
        for i, (images, labels, _, _) in enumerate(loader):
            ...
            if opts.infer_branch == 'both':
                ...
                # pred1 = outputs1.detach().max(dim=1)[1]
                # logit = torch.sigmoid(outputs2_past)
                # preds = logit[:, 1:].argmax(dim=1) + 1  # pred: [N. H, W]
                # preds[pred1 == 0] = 0
...
def main(opts):
    ...
    model = model_map[opts.model](num_classes=opts.num_classes, output_stride=opts.output_stride,
                                  bn_freeze=opts.bn_freeze, with_meta=opts.meta)
