...
class DGSegmentation(data.Dataset):
    cmap = deepglobe_cmap()
    ...
    def __init__(self,
        ...
        dg_root = './datasets/data/deepglobe'

        # data root数据路径 != ade_root 数据txt路径
        image_dir = os.path.join(self.root, 'land_train')
        mask_dir = os.path.join(self.root, 'onechannel_label')

        assert os.path.exists(mask_dir), "annotations not found"

        self.target_cls = get_tasks('deepglobe', self.task, cil_step)
        self.target_cls += [255]  # including ignore index (255)

        if image_set == 'test' or image_set == 'val':
            for s in range(cil_step):
                self.target_cls += get_tasks('deepglobe', self.task, s)

        if image_set == 'test':
            file_names = open(os.path.join(dg_root, 'val.txt'), 'r')
            ...
            file_names = get_dataset_list('deepglobe', self.task, cil_step, image_set, self.overlap)
            # 写一个新的txt
        self.images = [os.path.join(image_dir, x) for x in file_names]
        self.masks = [os.path.join(mask_dir, x.replace('_sat.jpg', '_label.png')) for x in file_names]
        self.file_names = file_names

        # class re-ordering
        all_steps = get_tasks('deepglobe', self.task)  # [[],[],...]
        all_classes = []  # [0,1,2,...]
