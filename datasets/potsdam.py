...
class PdSegmentation(data.Dataset):
    ...
    def __init__(self,
        ...
        self.target_cls = get_tasks('potsdam', self.task, cil_step)
        ...
        if image_set == 'test' or image_set == 'val' or image_set == 'test_val':
            for s in range(cil_step):
                self.target_cls += get_tasks('potsdam', self.task, s)
        ...
        if image_set == 'test':
            ...
            file_names = get_dataset_list('potsdam', self.task, cil_step, image_set, self.overlap)
            # 写一个新的txt
        ...
        # class re-ordering
        all_steps = get_tasks('potsdam', self.task)  # [[],[],...]
        all_classes = []  # [0,1,2,...]
